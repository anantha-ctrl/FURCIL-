...
### 🐾 Latest wave — real-time database sync, robust admin CRUD, strict payment verification & live QR scan delivery updates
- **Live QR Delivery & Product Verification** — public verification endpoint (`/api/orders/verify/{id}`) upgraded to support lookups by numeric ID *or* order number (`FUR00009`), with case-insensitive token hash validation and 5-second live polling. Scanning an invoice QR code displays live delivery status (`Order Delivered ✓`, `Shipped / In Transit 🚚`, `Processing`, `Pending`), shipment carrier/tracking #, recipient address, item thumbnails, and a strict payment verification badge (**PAID (VERIFIED ✓)** vs **PENDING (UNVERIFIED)**).
...
- **Per-product returnable flag** — each product carries an admin-set Returnable toggle (`products.is_returnable`, default yes). Turn it off for items that can't be returned (opened food, hygiene products). Shows a Returnable/No-return chip in the admin product list and product page.
- **Total-based payment rule** — orders up to `cod_max_amount` (₹1,000) may pay COD or online; above it, only Razorpay is offered.
